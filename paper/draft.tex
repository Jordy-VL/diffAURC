...
What is the formula for $\alpha_i$? <TBD> \\
...
\begin{align}
    \frac{1}{n} \sum_{i=1}^{n-1} \frac{\sum_{j=i+1}^n\ell([f(x_{i:n})]_{1:k},y_j)}{(n-i)}\label{eq:aurcloss_n2}
\end{align}

$\mathcal{O}(n)$ algorithm for computing all $\alpha_i$ given an index $\{\pi_i\}_{1\leq i\leq n}$ that sorts $\{f(x_i)_{k+1}\}_{1\leq i \leq n}$ in descending order.

%O(N) version with descending sort and assumes zero-based numbering
\begin{align}
    \frac{1}{n} \sum_{i=1}^{n} \frac{1}{i} \sum_{j=0}^{i-1}\ell([f(x_{\pi_j})]_{1:k},y_{\pi_j})
    \label{eq:aurcloss_n}
\end{align}
