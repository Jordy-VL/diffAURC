...
$\mathcal{O}(n)$ algorithm for computing all $\alpha_i$ given an index $\{\pi_i\}_{1\leq i\leq n}$ that sorts $\{f(x_i)_{k+1}\}_{1\leq i \leq n}$?
...
\begin{align}
    \frac{1}{n} \sum_{i=1}^{n-1} \frac{\sum_{j=i+1}^n\ell([f(x_{i:n})]_{1:k},y_j)}{(n-i)}
\end{align}
...
\subsection{Literature review on differentiable \AURC}

\cite{cao2022generalizing} close in spirit, dependent on cost, similarly defines it as a $K+1$ classification problem. 
